...
---------
  ...
  * size = edge-scaled, expanding, no look-ahead. Bets are walked in time order
    ...
    BEFORE it, multiplier = 1 + --size-slope * z clipped to [--size-min,
    --size-max], normalised by the running average multiplier so the average
    size tracks --contracts (default 100). Nothing about the rest of the season
    is used. The first --size-warmup bets of a season are sized flat, and the
    realised average lands near --contracts rather than exactly on it.
    --size-slope 0 restores flat sizing. Sizes are NOT
...
def main():
    ...
    n, zscore = edge_scaled_size(
        b["edge"].to_numpy("float64"),
        b["fill_ts"].to_numpy("float64"),
        float(args.contracts), args.size_slope, args.size_min, args.size_max,
        args.size_warmup,
        b["bt_season"].to_numpy() if "bt_season" in b.columns else None)
    if args.cap_to_print:
        n = np.minimum(n, np.nan_to_num(b["fill_count"].to_numpy("float64")))
    n = np.floor(n)
    b["edge_z"] = zscore
    print(f"[size] edge-scaled expanding (no look-ahead), avg ~{args.contracts} contracts"
          + (" capped at the printed size" if args.cap_to_print else " (uncapped)")
          + f"; slope {args.size_slope:g}, multiplier clipped to "
            f"[{args.size_min:.2f}, {args.size_max:.2f}], first "
